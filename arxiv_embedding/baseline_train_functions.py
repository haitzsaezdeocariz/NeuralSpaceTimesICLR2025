...
# Import Closed Form Spacetime Models: Minkowski, DeSitter
from closed_form_spacetimes import *
...
def train_baseline(
    ...
    if model_type == 'des':
        metric = DeSitterNeuralNetwork(
            N = feature_dim, #input dimension
            D = space_dim+1, # Space Dimensionality
            T = time_dim, # Time Dimensionality
            J_encoder = 10, # Depth of encoder
        )
    ...
    for epoch in range(num_epochs):
        ...
        for i, data in enumerate(train_loader):
            ...
            # Create mask for non-infinite values
            valid_mask = ~torch.isinf(distortion_batch)
            # Filter out inf values
            distortion_batch = distortion_batch[valid_mask]
            distortion_batch = distortion_batch.unsqueeze(1)
            # For the append operation, also filter inf values
            distortion.append((masked_labels/masked_distance)[~torch.isinf(masked_labels/masked_distance)])
